    ...
    cy.fixture('language-head.json').then((languageHeadFixture) => {
      cy.get('main').within(($main) => {
        cy.root()
          .should(
            'contain',
            `You've answered this word correctly ${languageHeadFixture.correctCount} times.`
          )
          .and(
            'contain',
            `You've answered this word incorrectly ${languageHeadFixture.incorrectCount} times.`
          );
      });
    });
  });
