    ...
    it(`displays score and feedback the word was incorrect`, () => {
      ...
      Cypress.Promise.all([
        ...
        cy.get('main').within(($main) => {
          ...
          cy.get('h4').should('have.text', `Good try, but not quite right :(`);
          cy.get('.DisplayFeedback p').should(
            'have.text',
            `The correct translation for "${incorrectFixture.original}" was "${incorrectFixture.translation}" and you chose "${incorrectFixture.guess}".`
          );
    ...
    it(`gives feedback the word was correct`, () => {
      ...
      Cypress.Promise.all([
        ...
        cy.get('main').within(($main) => {
          cy.get('.DisplayScore p').should(
            'have.text',
            `Your total score is: ${correctFixture.total_score}`
          );
          cy.get('h4').should('have.text', `You were correct! :D`);
          cy.get('.DisplayFeedback p').should(
            'have.text',
            `The correct translation for "${correctFixture.original}" was "${correctFixture.translation}" and you chose "${correctFixture.guess}".`
          );
          cy.get('button').should('have.text', `Try another word!`);
        });
      });
